...
applyrules(Client *c)
{
	...
	if (c->isfloating) {
		...
		if (matched_rule && matched_rule->width > 0)  c->geom.width  = matched_rule->width;
		if (matched_rule && matched_rule->height > 0) c->geom.height = matched_rule->height;
...
{
	Client *tmp;
	Client *c = focustop(selmon); /* Correct function to grab the focused client */
