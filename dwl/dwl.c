...
static void dwindle(Monitor *m);       /* dwindle patch */
static void movestack(const Arg *arg); /* dwindle patch: Move Windows */
...
void applyrules(Client *c) {
  ...
  int i;
  const Rule *r;
  const Rule *matched_rule = NULL; /* 1. Create a pointer to hold the match */
  ...
  for (r = rules; r < END(rules); r++) {
    if ((!r->title || strstr(title, r->title)) &&
      ...
      matched_rule = r; /* <-- CRITICAL: YOU NEED THIS LINE HERE */
  ...
  /* --- ADD THIS BLOCK TO CENTER FLOATING WINDOWS --- */
  if (c->isfloating) {
    /* 3. Use matched_rule instead of r */
    if (matched_rule && matched_rule->width > 0)
      c->geom.width = matched_rule->width;
    if (matched_rule && matched_rule->height > 0)
      c->geom.height = matched_rule->height;

    /* Center the window perfectly using its finalized geometry dimensions */
    c->geom.x = mon->m.x + (mon->m.width - c->geom.width) / 2;
    c->geom.y = mon->m.y + (mon->m.height - c->geom.height) / 2;
...
void cleanup(void) {
  ...
  if (fork() == 0) {
    setsid();
    execvp("systemctl", (char *const[]){"systemctl", "--user", "stop",
                                        "dwl-session.target", NULL});
    exit(1);
  }
...
void run(char *startup_cmd) {
  ...
  if (fork() == 0) {
    ...
    if (import_pid == 0) {
      execvp("systemctl",
             (char *const[]){"systemctl", "--user", "import-environment",
    ...
    /* Second: start target */
    execvp("systemctl", (char *const[]){"systemctl", "--user", "start",
                                        "dwl-session.target", NULL});
...
void movestack(const Arg *arg) {
  Client *tmp;
  Client *c =
      focustop(selmon); /* Correct function to grab the focused client */
  Client *p = NULL, *n = NULL;

  if (!c || c->isfloating)
    return;

  /* Find previous and next visible, non-floating clients */
  wl_list_for_each(tmp, &clients, link) {
    if (tmp->isfloating || !VISIBLEON(tmp, c->mon))
      continue;
    if (tmp == c) {
      /* Capture the next item if it exists */
      struct wl_list *next_link = c->link.next;
      while (next_link != &clients) {
        Client *next_c = wl_container_of(next_link, next_c, link);
        if (!next_c->isfloating && VISIBLEON(next_c, c->mon)) {
          n = next_c;
          break;
        }
    ...
    p = tmp; /* Keep tracking the last valid client before 'c' */
  }

  if (arg->i > 0 && n) {
    /* Move window down: Insert it after the next valid window */
    wl_list_remove(&c->link);
    wl_list_insert(&n->link, &c->link);
  } else if (arg->i < 0 && p) {
    /* Move window up: Insert it before the previous valid window */
    wl_list_remove(&c->link);
    wl_list_insert(p->link.prev, &c->link);
  } else {
    return;
  }
...
// {
// 	Client *c = selclient(), *p = NULL, *n = NULL;
// 	if (!c || c->isfloating)
// 		return;
//
// 	/* Find previous and next visible, non-floating clients */
