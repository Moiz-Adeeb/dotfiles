...
function helper.show_popup(application_list)
	...
	local cmd = string.format(
		[[foot -o colors-dark.background=002b36 -o colors-dark.alpha=0.9 --title popup -a "%s" bash -c "echo -e \"%s\"; read -n 1"]],
		x.win.ex_sm,
		msg
	)
...
function helper.open_application(application_list)
	...
	hl.bind(x.enter, function()
		hl.dispatch(hl.dsp.exec_cmd("hyprctl dismissnotify"))
		hl.dispatch(hl.dsp.exec_cmd("pkill -f popup"))
		hl.dispatch(hl.dsp.submap("reset"))
	end)
	hl.bind(x.esc, function()
		hl.dispatch(hl.dsp.exec_cmd("hyprctl dismissnotify"))
		hl.dispatch(hl.dsp.exec_cmd("pkill -f popup"))
		hl.dispatch(hl.dsp.submap("reset"))
	end)
...
function helper.submap(shortcut, application_type, submap_name)
